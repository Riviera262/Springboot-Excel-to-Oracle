...
public class DataGenerator {
    ...
    public static void main(String[] args) {
        ...
        // 2. Tạo 1,000,000 dòng dữ liệu mẫu
        System.out.println("Đang bắt đầu tạo dữ liệu...");
        for (int i = 1; i <= 1000000; i++) {
            Row row = sheet.createRow(i);

            row.createCell(0).setCellValue("TRX" + String.format("%07d", i)); // Ví dụ: TRX0000001
            ...
            // Tạo số tiền ngẫu nhiên từ 1000.00 đến 1000000.00 (không có dấu phân cách hàng nghìn)
            double amount = 1000 + (1000000 - 1000) * random.nextDouble();
            // Dùng String.format để ép về định dạng số có 2 chữ số thập phân, không dấu phẩy
            String amountStr = String.format(Locale.US, "%.2f", amount);

            // Ghi vào Cell dưới dạng String (Text)
            row.createCell(3).setCellValue(amountStr); // Lưu trực tiếp dưới dạng số, không format string
        ...
        // 3. Xuất file
        try (FileOutputStream fileOut = new FileOutputStream("MB_Test_1M.xlsx")) {
            workbook.write(fileOut);
            workbook.dispose(); // Xóa file tạm của SXSSF
            System.out.println("Tạo file thành công! Tên file: MB_Test_1M.xlsx");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
